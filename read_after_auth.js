                    ...
                    if (err) {
                        ...
                        */          
						//SERI NUMARASINI ALMAK ICIN BU KOMUTU KULLANIYORUZ
							...
							var messageAuthenticate = new Buffer([0xFF, 0x82, 0x00, 0x00, 0x06, 
							...
							reader.transmit(messageAuthenticate, 40, protocol, function(err, dataAuthenciate) {
								if (err) {
									...
									console.log("AUTHENTICATION OK1!!!");
									console.log(dataAuthenciate);
									
									var messageAuthenticate2 = new Buffer([0xFF, 0x86, 0x00, 0x00, 0x05, 
																		   0x01, 0x00, readBlock, 0x60, 0x00]);
									  
									reader.transmit(messageAuthenticate2, 40, protocol, function(err, dataAuthenciate2) {
										if (err) {
											console.log('Error(', reader.name, '):', err);
										}
										else {	
											console.log("AUTHENTICATION OK2!!!");
											console.log(dataAuthenciate2);
										}
